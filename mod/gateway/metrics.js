...
exports.metrics = {
  ...
  motion : { name:'M', regexp:/\bMOTION\b/i, value:'MOTION', pin:1, graph:1, logValue:1, graphValSuffix:' detected!', graphOptions:{ legendLbl:'Motion', lines: { show:false, fill:false }, points: { show: true, radius: 5, lineWidth:1 }, grid: { backgroundColor: {colors:['#000', '#03c', '#08c']}}, yaxis: { ticks: 0 }}},
    ...
    safeArray : { name:'SARR', regexp:/\bsafeArray:([\d:,]+)\b/i, value:''},
    detectArray : { name:'DARR', regexp:/\bdetectArray:([\d:,]+)\b/i, value:''},
    threat : { name:'TD', regexp:/\bthreat:(\d+)\b/i, value:'', pin:1 },
    threatCount : { name:'TC', regexp:/\bthreatCount:(\d+)\b/i, value:'', pin:1 },
    threatSignal : { name:'TSIG', regexp:/\bthreatSignal:([\d:,]+)\b/i, value:''},
    isSafe : { name:'SD', regexp:/\bsafe:(\d+)\b/i, value:''},
    imageID : { name:'IMAGE_ID', regexp:/\bimageId:(\d+)\b/i, value:''},
    serialNum : { name:'SN', regexp:/\bserialNum:(\d+)\b/i, value:''},
    collectionNum : { name:'CN', regexp:/\bcollectionNum:(\d+)\b/i, value:''},
    minPixel : { name:'MINPIX', regexp:/\bminPixel:(\d+)\b/i, value:''},
    maxPixel : { name:'MAXPIX', regexp:/\bmaxPixel:(\d+)\b/i, value:''},
    avgPixel : { name:'AVGPIX', regexp:/\bavgPixel:(\d+)\b/i, value:''},
    pixelCount : { name:'PIXCOUNT', regexp:/\bpixelCount:(\d+)\b/i, value:''},
    repeatCount : { name:'REPCOUNT', regexp:/\bthisFrameCount:(\d+)\b/i, value:''},
    totalCount : { name:'TOTCOUNT', regexp:/\btotalFrameCount:(\d+)\b/i, value:''},
    statusCount : { name:'STATUS_COUNT', regexp:/\bstatusCount:(\d+)\b/i, value:''},
    alertCount : { name:'ALERT_COUNT', regexp:/\balertCount:(\d+)\b/i, value:''},
    imageCount : { name:'IMAGE_COUNT', regexp:/\bimageCount:(\d+)\b/i, value:''},
    vmin : { name:'VMIN', regexp:/\bVMIN:([\d\.]+)\b/i, value:''},
    vmax : { name:'VMAX', regexp:/\bVMAX:([\d\.]+)\b/i, value:''},
    amin : { name:'AMIN', regexp:/\bAMIN:([\d\.]+)\b/i, value:''},
    amax : { name:'AMAX', regexp:/\bAMAX:([\d\.]+)\b/i, value:''},
    
  lastMotion : { name:'LO', regexp:/(?:LO|LM)\:((?:\d+h)?\d{1,2}m|\d{1,2}s)/i, value:'', pin:1 },
  intMotion : { name:'IM', regexp:/\bIMOTION:(\d+)\b/i, value:'', pin:1},
  ...
  A : { name:'A', regexp:/\b(?:A?)\:([\d\.]+)?\b/i, value:'', duplicateInterval:3600, unit:'A', graph:1, graphOptions:{ legendLbl:'Amps', lines: { fill:true, lineWidth:1 }, grid: { backgroundColor: {colors:['#000', '#03c', '#08c']}}, yaxis: { min: 0, autoscaleMargin: 0.25, autoscaleBottom:false }}},
...
exports.events = {
  ...
  threatSMS : { label:'Threat : SMS', icon:'comment', descr:'Send SMS when Threat is detected', serverExecute:function(node) { if (node.metrics['TD'] && node.metrics['TD'].value > 0 && (Date.now() - new Date(node.metrics['TD'].updated).getTime() < 20000)) { sendSMS('THREAT DETECTED', 'THREAT WAS DETECTED ON NODE: [' + node._id + ':' + node.label.replace(/\{.+\}/ig, '') + '] @ ' + new Date().toLocaleTimeString()); }; } },

    highMotionSMS : { label:'IntMotion : SMS', icon:'comment', descr:'Send SMS when HIGHMOTION is detected', serverExecute:function(node) { if (node.metrics['IM'] && node.metrics['IM'].value >= 4 && (Date.now() - new Date(node.metrics['IM'].updated).getTime() < 2000)) { sendSMS('HIGH MOTION DETECTED', ' multiple events detected on node: [' + node._id + ':' + node.label.replace(/\{.+\}/ig, '') + '] @ ' + new Date().toLocaleTimeString()); }; } },
